...
const init = async () => {
 ...
 const opportunityAddress = '0x298D999e947B3151Af705522217EFb181E71206e'
  ...
  result = await opportunity.markets(TUSD);
  ...
  try {
    ...
    result = await TUSDSigned.approve(opportunityAddress, tokens(9990));
    receipt = await result.wait();
    console.log(receipt);

    result = await opportunity.supply(TUSD, ethers.utils.parseEther('10'), true, options);
    console.log(result);
    receipt = await result.wait();
    console.log(receipt);

    balanceAvailable = await opportunity.getBalance(TUSD)
