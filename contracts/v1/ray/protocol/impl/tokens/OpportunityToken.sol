...
/// @notice  OT's are an ERC721 compatible token that represent a ray's 'stake' or
///          position in a particular Opportunity pool. That is, in the low-level pools.
///
/// @dev    The OT is meant to be eternal. We don't want to need to upgrade
...
/// Author:  Devan Purhar
/// Version: 1.0.0
contract OpportunityToken is ERC721Burnable, ERC721Metadata {
  ...
  /*************** STORAGE VARIABLE DECLARATIONS **************/
    ...
    modifier onlyTokenWrapper()
    {
        require(
            ...
            "#OpportunityToken onlyTokenWrapper Modifier: Only Token Wrapper can call this"
        );
    ...
    /// @notice Constructs an ERC721 RAYOT contract. Sets the name to be
    ///         "RAY Opportunity Token" and the ticker to "RAYOT".
    ///
    /// @param   __storage - The Storage contracts address
    constructor(address __storage)
		  ERC721Metadata("RAY Opportunity Token", "RAYOT")
		  public
	  {
    ...
    /// @notice  Mint the unique RAYOT and insert it into the token registry.
    ///
    ...
    ///
    /// @return  the token id of the minted RAYOT
    ...
    /// @notice  Checks if a RAYOT with the specified ID exists
    ///
    /// @dev     This is used to verify a token Id being entered for mutation
    ///          in other contracts actually exists
    ///
    ...
    /// @notice  Helper function for computing the token id of a RAYOT for issuance.
    ///
